...
from ..config import TELEGRAM_BOT_TOKEN, TELEGRAM_LOCAL_API_URL, ensure_dirs
...
async def _post_init(application) -> None:
    ensure_dirs()
    application.create_task(progress.run(application.bot))
...
def main() -> None:
    ...
    builder = ApplicationBuilder().token(TELEGRAM_BOT_TOKEN).post_init(_post_init)
